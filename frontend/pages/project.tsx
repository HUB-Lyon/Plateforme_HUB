...
export const getTitleStatus = (status: string): string => {
    if (status === 'Pending' || status === 'in Progress' || status === 'Done' || status === 'Archived' || status ==='Refused' || status === 'req finish')
        return status;
    return 'N/A';
};
                        ...
                        return (
                            <li key={idx} className="m-4 flex flex-col justify-between relative rounded-xl bg-white text-black shadow-xl transition ease-in-out delay-100 hover:scale-105 duration-250">
                                <Link href={`/project/${project.id}`} title={project.name}>
                                    <Image src={project.image} alt={project.name} width={300} height={300} className="object-cover rounded-t-lg mx-auto w-full"></Image>
